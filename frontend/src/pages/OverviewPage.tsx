...
/* ─── KPI Card ─── */
function KpiCard({
  label, value, icon, progress,
}: {
  label: string; value: string; icon: string; progress: number;
}) {
  return (
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard label="虚机总数" value={String(d.vmTotal)} icon="memory" progress={d.vmTotal > 0 ? (d.idleTotal / d.vmTotal) * 100 : 0} />
        <KpiCard label="闲置资源" value={String(d.idleTotal)} icon="developer_board" progress={d.vmTotal > 0 ? (d.idleTotal / d.vmTotal) * 100 : 0} />
        <KpiCard label="今日 RDP 登录" value={String(d.todayRdp)} icon="swap_calls" progress={d.todayRdp > 0 ? 45 : 0} />
